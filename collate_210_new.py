# Modified from the original, which was a cleanup of Will Chen's code, to keep
# all readout columns and use the same output filename convention as the newer
# projects.
...
for replica in df:
    ...
    with open(output_path.child('Replicate_'+replica[1]+'.csv'), 'w') as fp:
        cols = finaldf[replica].columns
        cols = [non_ascii(col) for col in cols]
        finaldf[replica].columns = cols
        finaldf[replica].to_csv(fp, index=False)

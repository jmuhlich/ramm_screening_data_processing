...
rep_paths = [p for p in input_path.listdir() if p.isdir() and p.endswith(']')]

df = {}

print "Reading experimental design\n==========\n"

layout_filename = 'Plate layout_backgroundcompounds_Rep1+2.xlsx'
...
for replicate, rdf in df.items():
    ...
    for panel_a, panel_b in (('Sim_000001', 'Sim_000002'),):
    ...
    assert final.shape[0] == 240*7, "wrong number of rows"
    assert final.shape[1] == 695, "wrong number of columns"

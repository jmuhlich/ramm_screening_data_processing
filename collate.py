...
platefilelist = ['Plate layout_1-18.csv','Plate Layout_19-38.csv','Plate layout_39-57.csv','Plate Layout_58-73.csv']
  ...
  for subdir2 in immediate_subdir_2 :
    ...
    for csvfile in csvfilelist :
      ...
      if sim == 'Sim_000001' or sim == 'Sim_000002' :
        ...
        wt = pd.DataFrame([str(exptime) for i in range(0,240)], index=tempdf.index, columns = ['WallTime'])
        tempdf = pd.concat([wt,tempdf],axis=1)
        rp = pd.DataFrame([replicaname for i in range(0,240)], index=tempdf.index, columns = ['ReplicaNumber'])
